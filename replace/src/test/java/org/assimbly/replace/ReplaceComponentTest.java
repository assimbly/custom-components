...
class ReplaceComponentTest extends CamelTestSupport {
        ...
        return new RouteBuilder() {
            ...
            public void configure() throws Exception {
                ...
                from("direct:executeSingleGroup")
                        .to("replace://?regex=" + regex + "&replaceWith=" + replaceWith + "&group=0")
                        .to("mock:out");

                from("direct:executeMultipleGroupsWithFlag")
                        .to("replace://?regex=" + multipleGroupRegex2 + "&replaceWith=" + replaceWith + "&group=0&flags=i")
                        .to("mock:out");

                from("direct:executeWithDotall")
                        .to("replace://?regex=RAW(" + dotallRegex + ")&replaceWith=RAW( * )&group=0&flags=s")
                        .to("mock:out");

                from("direct:executeWithoutDotall")
                        .to("replace://?regex=" + dotallRegex + "&replaceWith=" + " * " + "&group=0")
                        .to("mock:out");

                from("direct:executeMultipleGroups")
                        .to("replace://?regex=" + multipleGroupRegex + "&replaceWith=" + replaceWith + "&group=2")
                        .to("mock:out");

                from("direct:executeWithHeader")
                        .setHeader("replace", new ConstantExpression("kabisa"))
                        .to("replace://?regex=" + multipleGroupRegex + "&replaceWith=" + replaceWithHeader + "&group=0")
                        .to("mock:out");

                from("direct:executeWithExceptionalCharacters")
                        .to("replace://?regex=" + endOfLineRegex + "&replaceWith=\\n&group=0")
                        .to("mock:out");

                from("direct:executeWithExceptionalCharacterInHeader")
                        .setHeader("replace", new ConstantExpression("\\n"))
                        .to("replace://?regex=" + endOfLineRegex + "&replaceWith=" + replaceWithHeader + "&group=0")
                        .to("mock:out");

                from("direct:executeWithWhiteSpaceCharacter")
                        .to("replace://?regex=" + whiteSpaceRegex + "&replaceWith=" + " " + "&group=0")
                        .to("mock:out");

                from("direct:executeWithMultiTypeLineEndings")
                        .to("replace://?regex=" + replaceWith + "&replaceWith=" + "Assimbly" + "&group=0")
                        .to("mock:out");
    ...
    @Test
    void testMultipleGroupReplaceWithFlag() {
    ...
    @Test
    void testUnescapingOfExceptionalCharacters() { //We want to unescape \n \r \t
    ...
    @Test
    void testUnescapingOfExceptionalCharactersInHeader() {
    ...
    @Test
    void testReplaceWhiteSpaceChar() {
    ...
    @Test
    void testBitwiseReductionForRegexMagicConstant() {
