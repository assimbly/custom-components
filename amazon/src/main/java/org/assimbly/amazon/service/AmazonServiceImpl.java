...
import org.assimbly.amazon.comparator.ParameterComperator;
...
public class AmazonServiceImpl implements AmazonService {
    ...
    private static final String MARKETPLACE_OVERRIDE_HEADER = "AssimblyAmazonMarketPlaceId";
    ...
    Map<String, String> applyOverrideHeaders(Map<String, String> parameters, Map<String, Object> headers) {
        Map<String, Object> overrideHeaders =
                headers.entrySet().stream()
                        .filter(header -> Pattern.matches(header.getKey(), "AssimblyAmazon*"))
                        .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
        ...
        parameters.entrySet().stream()
                .filter(entry -> overrideHeaders.containsKey(
                        String.format("AssimblyAmazon%s", CaseFormat.LOWER_CAMEL.to(CaseFormat.UPPER_CAMEL, entry.getKey()))
                ))
                .forEach(entry -> {
