...
import world.dovetail.soap.domain.SoapAttribute;
import world.dovetail.soap.domain.SoapHeader;
import world.dovetail.soap.util.helpers.*;
...
public class SoapProcessor implements Processor {
    ...
    public void process(Exchange exchange) throws Exception {
        ...
        if (config.isSmart()) {
            if (in.getHeader("DovetailSoapDestination") != null)
                destination = in.getHeader("DovetailSoapDestination", String.class);
    ...
    void applyOverrideHeaders(CamelContext context, Map<String, Object> headers, SoapConfiguration configuration) {
        ...
        headers.entrySet().stream()
            .filter(header -> EndpointHelper.matchEndpoint(context,header.getKey(), "DovetailSoap*"))
            .forEach(header -> {
                String prop = header.getKey().replace("DovetailSoap", "");
