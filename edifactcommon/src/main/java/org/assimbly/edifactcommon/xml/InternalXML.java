...
import org.assimbly.edifactcommon.functions.Lang;
...
import org.assimbly.edifactcommon.mapforce.MFSingletonSequence;
...
{
	public static String XMLToString( IMFNode node ) throws Exception
	{
		Document doc = XmlTreeOperations.createDocument();
        MFDOMWriter.write(new MFSingletonSequence(node), doc);
		return XmlTreeOperations.saveXml( doc, false, true );
	}

	public static String XMLToGroupingKey( IEnumerable e) throws Exception
	{
	...
	public static String XMLToGroupingKey( IMFNode node) throws Exception
	{
		StringBuffer sBuf = new StringBuffer();
		write(new MFSingletonSequence(node), sBuf);
		return sBuf.toString();
	}
	...
	public static byte[] XMLToBase64Binary( IMFNode node, String encoding, String byteorder, boolean bBOM ) throws Exception
	{
		Document doc = XmlTreeOperations.createDocument();
        MFDOMWriter.write(new MFSingletonSequence(node), doc);
	...
	public static IMFNode XMLFromString( String xmlString ) throws Exception
	{
		return new DOMNodeAsMFNodeAdapter(XmlTreeOperations.loadXml(xmlString));
	}

	public static IMFNode XMLFromBase64Binary( byte[] xmlString ) throws Exception
	{
	...
	{
		Document faultDoc = XmlTreeOperations.createDocument();
		MFDOMWriter.write(detail.select(IMFNode.MFQueryKind_All, null), faultDoc);
		throw new UserException( message, faultDoc );
	}
}
