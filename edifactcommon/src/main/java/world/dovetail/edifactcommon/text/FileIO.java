...
public class FileIO {
	...
	public FileIO(InputStream inStream, String encoding, boolean bBigEndian, boolean bBOM)
	{
		m_inStream = inStream;
		m_outStream = null;
		m_Encoding = encoding;
	...
	public FileIO(OutputStream outStream, String encoding, boolean bBigEndian, boolean bBOM)
	{
    ...
    public StringBuffer readToEnd() throws IOException 
	{
		...
		if (m_inStream != null)
    	{
			...
			{
				if( enc_size == 1 ) {
					LOG.warn("Warning: input encoding is UTF-8, not '" + m_Encoding + "'");
					m_Encoding = "UTF-8";
					m_Charset = Charset.forName( formNativeEncodingName( m_Encoding, m_bBigEndian ) );
					unisize = 1;
				} else
				if( enc_size == 2 ) {
					LOG.warn("Warning: input encoding is UTF-16 or UCS-2, not '" + m_Encoding + "'");
					m_Encoding = "UTF-16";
			...
			{
				LOG.warn("Warning: input byte order is " + getByteOrderName(bIsBigEndian) + ", not " + getByteOrderName(m_bBigEndian));
				m_bBigEndian = bIsBigEndian;
				m_Charset = Charset.forName( formNativeEncodingName( m_Encoding, m_bBigEndian ) );
			}
